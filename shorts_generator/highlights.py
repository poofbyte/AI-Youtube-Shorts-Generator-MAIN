...
def _parse_json_loose(raw: str) -> Any:
    """gpt-5-4 / Gemini sometimes wrap JSON in markdown fences — strip and parse."""
    text = raw.strip()
...
def _coerce_int(value: object, default: int = 0) -> int:
    f = _coerce_float(value, default=float(default))
    return int(round(f))


def _sanitize_highlights(raw_highlights: object, duration: float = float("inf"), offset: float = 0.0) -> List[Dict]:
    ...
    for item in raw_highlights:
        ...
        start_val = item.get("start_time") if item.get("start_time") is not None else (item.get("start") if item.get("start") is not None else item.get("startTime"))
        end_val = item.get("end_time") if item.get("end_time") is not None else (item.get("end") if item.get("end") is not None else item.get("endTime"))
        ...
        # If LLM output relative timestamps for a chunk at offset, adjust to absolute
        if offset > 0 and start < offset:
            start += offset
            end += offset
        ...
        raw_score = _coerce_float(item.get("score"), default=8.0)
        if 0 < raw_score <= 10.0:
        ...
        cleaned.append(
            {
                "title": str(item.get("title") or item.get("hook") or "Untitled Highlight").strip(),
                "start_time": start,
                "end_time": end,
                "score": score,
                "hook_sentence": str(item.get("hook_sentence") or item.get("hook") or "").strip(),
                "virality_reason": str(item.get("virality_reason") or item.get("reason") or "").strip(),
            }
        )
    ...
    for attempt in range(1, MAX_HIGHLIGHT_API_ATTEMPTS + 1):
        ...
        try:
            ...
            if isinstance(parsed, list):
                ...
                raw_list = parsed.get("highlights") or parsed.get("clips") or parsed.get("data") or parsed.get("results") or []
            else:
                raw_list = []
            highlights = _sanitize_highlights(raw_list, duration=duration, offset=offset)
            if highlights:
                return {"highlights": highlights}
            last_error = f"no valid highlights in response (raw output snippet: {str(raw)[:150]!r})"
...
def get_highlights(
    ...
    if duration >= LONG_VIDEO_THRESHOLD:
        ...
        for i, chunk in enumerate(chunks):
            ...
            result = call_highlight_api(text, content_info, duration, num_clips=num_clips, is_chunk=True, offset=offset, llm_fn=llm_fn)
