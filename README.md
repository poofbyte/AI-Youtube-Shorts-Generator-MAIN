...
4. **Set up environment variables:**
   ...
   LOCAL_WHISPER_MODEL=tiny          # tiny / base / small / medium / large-v3 (tiny is recommended for low-end PCs)
...
### Single video (Local mode — free, offline transcription + cloud/local LLM)
...
### Low-End PC & Zero-Cost Cloud LLM Setup

If running on a low-end PC, configure `--mode local` with Google Gemini's Free Tier:
1. Set `LLM_PROVIDER=gemini` and provide a free `GEMINI_API_KEY` from Google AI Studio.
2. Set `LOCAL_WHISPER_MODEL=tiny` (~150MB RAM usage, fast CPU transcription).
3. All heavy LLM highlight analysis is offloaded to Google Cloud (0% PC CPU load), while audio transcription and video cropping run locally.
...
| Transcription | MuAPI `/openai-whisper` | `faster-whisper` (CPU or CUDA, cached as `.srt`) |
| Highlight LLM | MuAPI `gpt-5-mini` | `LLM_PROVIDER=openai` uses OpenAI (`gpt-4o-mini`), `LLM_PROVIDER=gemini` uses Gemini (`gemini-3.5-flash` with auto-retries & model fallback) |
| Vertical crop | MuAPI `/autocrop` | `ffmpeg` + OpenCV face tracking (with center-crop fallback & safe Windows file removal) |
